...
const initialSchedule: ScheduleItem[] = [
    { id: 1, client: 'Princewill Amadi', property: 'Skyline Terrace - Apt 4B', time: '10:30 AM', date: 'Oct 15', type: 'Initial Viewing' },
    { id: 2, client: 'Fatima Lawal', property: 'Palm Grove Villa', time: '02:00 PM', date: 'Oct 22', type: 'Final Inspection' },
];

const initialAppointments = [
    { id: 3, client: 'Olawale Johnson', time: '10:30 AM', date: 'Oct 28', type: 'Property Viewing' },
    { id: 4, client: 'Chidi Okafor', time: '02:00 PM', date: 'Oct 29', type: 'Virtual Consultation' },
];
...
export default function SchedulePage() {
    ...
    const [selectedDay, setSelectedDay] = React.useState<number | null>(27); // Default to today in mock

    const dayAppointments = React.useMemo(() => {
        if (!selectedDay) return [];
        const dateStr = `Oct ${selectedDay.toString().padStart(2, '0')}`;
        return [...schedule, ...appointments].filter(apt => apt.date === dateStr);
    }, [selectedDay, schedule, appointments]);
    ...
    return (
        <div className="space-y-10 pb-12">
            ...
            <div className="flex justify-between items-center">
                ...
                <button className="flex items-center gap-2 bg-brand-green text-white px-6 py-3 rounded-2xl font-semibold shadow-lg shadow-green-900/20 hover:bg-green-700 transition-all active:scale-95">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="font-semibold text-gray-900 uppercase tracking-widest text-xs">October 2023</h3>
                        ...
                        <div className="grid grid-cols-7 gap-2 text-center mb-4">
                            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, idx) => (
                                <span key={`${day}-${idx}`} className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">{day}</span>
                            ))}
                        </div>
                            ...
                            {Array.from({ length: 31 }).map((_, i) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`aspect-square rounded-2xl flex flex-col items-center justify-center relative cursor-pointer transition-all group ${selectedDay === day ? 'ring-2 ring-brand-green ring-offset-2' : ''
                                    ...
                                    >
                                        <span className={`text-sm font-semibold ${isToday ? 'text-white' : 'text-gray-900'}`}>{day}</span>
                                        {isBooked && !isToday && (
                                        ...
                                        {isBooked && (
                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 bg-gray-900 text-white text-[10px] p-2 rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-20">
                                                <p className="font-semibold">Booked Slot</p>
                    ...
                    <div className="space-y-6">
                        <div className="bg-brand-green text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden shadow-green-900/20">
                            <div className="relative z-10">
                                ...
                                <p className="text-2xl font-semibold">{dayAppointments.length} Pending Viewings</p>
                                <p className="text-xs text-white/70 mt-2 font-medium">
                                    {dayAppointments.length > 0
                                        ? `You have ${dayAppointments.length} appointments for October ${selectedDay}.`
                                        : `No appointments scheduled for October ${selectedDay}.`}
                                </p>
                            </div>
                        ...
                        <div>
                            <h3 className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest px-2 mb-4">
                                {selectedDay === 27 ? "Today's Agenda" : `Agenda for Oct ${selectedDay}`}
                            </h3>
                                        ...
                                        <div key={apt.id} className="bg-white p-5 rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-md transition-all group">
                                            <div className="flex gap-4 items-center">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-widest">{apt.date} • {apt.time}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-xs text-gray-400 px-2 italic">Nothing scheduled.</p>
                                )}
                            </div>
                        </div>
                    </div>
            ...
            {/* List View */}
            <section>
                <h2 className="text-xl font-semibold text-gray-900 mb-6">Upcoming Inspections</h2>
                        ...
                        <div key={item.id} className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-lg transition-all group">
                            <div className="flex gap-6 items-center">
                                <div className="bg-gray-50 p-4 rounded-3xl flex flex-col items-center justify-center min-w-[80px] border border-gray-100 group-hover:bg-brand-green group-hover:text-white transition-all">
                                    <span className="text-[10px] font-semibold text-gray-400 group-hover:text-white/60 uppercase tracking-widest leading-none mb-1">{item.date.split(' ')[0]}</span>
                                    <span className="text-2xl font-semibold leading-none">{item.date.split(' ')[1]}</span>
                                </div>
                                <div>
                                    <h3 className="font-semibold text-gray-900 text-lg">{item.client}</h3>
