...
interface CalculatorCTAProps {
  title?: string; // Made optional for minimal variant
  description?: string; // Made optional for minimal variant
  placeholder: string;
  redirectPath: string;
  ctaText?: string;
  variant?: 'default' | 'minimal'; // Added variant prop
}
...
export default function CalculatorCTA({
  ...
  variant = 'default', // Defaults to original style
}: CalculatorCTAProps) {
  ...
  // Minimal Variant: Just the input and button
  if (variant === 'minimal') {
    return (
  ...
  // Default Variant: Full section with headings
  return (
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <FadeIn direction="up" delay={0.1}>
          <div className="max-w-xl mx-auto flex flex-col md:flex-row gap-4 items-center">
            ...
            />
            <button onClick={handleRedirect} className="bg-brand-green text-white px-6 py-3 rounded-lg font-bold hover:bg-green-600 transition-colors shadow-md">
              {ctaText}
            </button>
          </div>
        </FadeIn>
