  ...
  return (
    ...
    >
      ...
      <div className="space-y-8 flex-1">
        ...
        <div>
          <label className="block text-sm font-bold text-gray-700 mb-4 uppercase tracking-wide">
            Years of experience as a caretaker?
          </label>
...
const CaretakerStep2 = () => {
  ...
  const [preferredPropertyTypes, setPreferredPropertyTypes] = useState<string[]>(
    caretakerData?.preferredPropertyTypes || []
  );
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {propertyTypes.map((type) => (
              ...
              >
                {preferredPropertyTypes.includes(type) && <FiCheck size={14} />}
                {type}
              </button>
            ))}
          </div>
...
const CaretakerStep3 = () => {
  const { getCurrentUser, updateRoleData, completeRoleOnboarding, goToStep, prevStep } = useOnboardingStore();
  const user = getCurrentUser();
  const caretakerData = user?.caretaker;
      ...
      <div className="flex-1">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {serviceOptions.map((service) => (
            ...
            >
              {service}
              {services.includes(service) && <FiCheck className="text-brand-green" size={18} />}
            </button>
          ))}
        </div>
      </div>
