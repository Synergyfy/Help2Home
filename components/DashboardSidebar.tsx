...
export default function DashboardSidebar({ isOpen = false, onClose }: { isOpen?: boolean, onClose?: () => void }) {
    ...
    const userRole = (Object.keys(NAV_CONFIG).find(role => pathname.includes(`/dashboard/${role}`)) || 'tenant') as keyof typeof NAV_CONFIG;
    const navItems = NAV_CONFIG[userRole];

    return (
        <aside className={`
      fixed inset-y-0 left-0 z-40 
      w-64 m-4 /* Adds spacing from the "wall" */
      bg-[#00853E] text-white 
      rounded-2xl /* Professional radius, not "too round" */
