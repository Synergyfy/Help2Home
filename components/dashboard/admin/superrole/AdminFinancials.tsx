            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Commission</span>
                    </div>
                    <div className="text-2xl font-black text-slate-900">₦12.8M</div>
                    <p className="text-slate-500 text-xs mt-1">Platform earnings @ 10% avg</p>
                </div>
                ...
                <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Pending Out</span>
                    </div>
                    <div className="text-2xl font-black text-slate-900">₦4.5M</div>
                    <p className="text-slate-500 text-xs mt-1">Awaiting admin clearance</p>
                </div>
            </div>
            ...
            <section className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-slate-100">
                    {pendingPayouts.map((payout) => (
                        <div key={payout.id} className="p-6 hover:bg-slate-50 transition-colors flex items-center justify-between">
                            <div className="flex items-start gap-4">
                                <div className="p-3 bg-slate-100 rounded-xl text-slate-600">
                                    <FiDollarSign size={20} />
                                </div>
                                <div>
                                    <h4 className="font-bold text-slate-900">{payout.recipient}</h4>
                                    <div className="flex items-center gap-3 mt-1">
                                        <span className="text-xs text-slate-500">{payout.method}</span>
                                        <span className="size-1 bg-slate-300 rounded-full" />
                                        <span className="text-xs text-slate-400 font-mono uppercase">{payout.id}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-8">
                                <div className="text-right">
                                    ...
                                    <div className="text-[10px] text-slate-400 font-black uppercase tracking-wider">{payout.requestedAt}</div>
                                </div>
