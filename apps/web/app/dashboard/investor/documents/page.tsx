        ...
        <FadeIn>
            <div className="space-y-10 pb-12">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <h1 className="text-3xl font-semibold text-gray-900 tracking-tight ">Documents Vault</h1>
                        <p className="text-gray-500 mt-1">Secure repository for all your legal and financial assets.</p>
                    </div>
                    <button className="flex items-center gap-2 px-8 py-4 bg-brand-green text-white font-semibold rounded-2xl text-sm  hover:shadow-xl hover:shadow-brand-green/20 transition-all">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {FOLDERS.map((folder) => (
                        <div key={folder.name} className="bg-white p-8 rounded-4xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 group cursor-pointer">
                            ...
                            <p className="text-xs text-gray-400 font-semibold uppercase tracking-widest">{folder.files} Items</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
                    <div className="p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gray-50/20">
                        <div className="flex items-center gap-4">
                            ...
                            <h3 className="text-xl font-semibold text-gray-900 tracking-tight">Recent Documents</h3>
                        </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-gray-50/50">
                                    <th className="px-8 py-5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Document Name</th>
                                    <th className="px-8 py-5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Type</th>
                                    <th className="px-8 py-5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Date Uploaded</th>
                                    <th className="px-8 py-5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-widest">File Size</th>
                                    <th className="px-8 py-5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Status</th>
                                    <th className="px-8 py-5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-widest"></th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={doc.id} className="group hover:bg-gray-50/50 transition-colors">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="font-semibold text-gray-900 group-hover:text-brand-green transition-colors">{doc.name}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 text-xs text-gray-500 font-semibold">{doc.type}</td>
                                        <td className="px-8 py-6 text-xs text-gray-500 font-medium">{doc.date}</td>
                                        <td className="px-8 py-6 text-xs text-gray-500 font-medium ">{doc.size}</td>
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-2">
                                                <div className={`size-2 rounded-full ${doc.status === 'Verified' ? 'bg-brand-green' : 'bg-orange-400'}`} />
                                                <span className={`text-[10px] font-semibold uppercase tracking-widest ${doc.status === 'Verified' ? 'text-brand-green' : 'text-orange-500'}`}>
                                                    {doc.status}
                                                </span>
                                            </div>
                                        </td>
                    ...
                    <div className="p-8 bg-gray-50/30 border-t border-gray-50 flex items-center gap-6">
                        <div className="flex items-center gap-2 text-brand-green bg-white px-4 py-2 rounded-full border border-brand-green/10 shadow-sm text-[10px] font-semibold uppercase">
