  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
      ...
      <div className="xl:col-span-2 space-y-6">
        <div className="bg-white p-4 rounded-xl border border-brand-green-100 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-center">
          ...
          <div className="flex gap-2">
            <select className="bg-brand-green-50 border border-brand-green-200 rounded-lg py-2 px-3 text-sm outline-none">
              <option>All Roles</option>
            </select>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-brand-green-100 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-brand-green-50 border-b border-brand-green-100">
                  <th className="py-4 px-6 text-xs font-bold uppercase text-brand-green-500">User</th>
                  <th className="py-4 px-6 text-xs font-bold uppercase text-brand-green-500">Assigned Roles</th>
                  <th className="py-4 px-6 text-xs font-bold uppercase text-brand-green-500">Status</th>
                  <th className="py-4 px-6 text-xs font-bold uppercase text-brand-green-500 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-brand-green-100">
                {users.map(user => (
                  <tr key={user.id} className="group hover:bg-brand-green-50/50 transition-colors">
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-brand-green-200 flex items-center justify-center font-bold text-brand-green-500">
                          {user.name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-bold text-sm text-brand-green-900">{user.name}</p>
                          <p className="text-xs text-brand-green-500">{user.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex justify-end gap-2">
                        <button className="p-1.5 text-brand-green-400 hover:text-blue-600 transition-colors"><HiOutlinePencilAlt size={18} /></button>
                        <button
                          onClick={() => removeSubAccount(user.id)}
                          className="p-1.5 text-brand-green-400 hover:text-red-600 transition-colors"
                        >
      ...
      <div className="xl:col-span-1">
        <div className="bg-white rounded-xl border border-brand-green-100 shadow-lg sticky top-8">
          <div className="p-6 border-b border-brand-green-50 bg-linear-to-r from-white to-brand-green-50 rounded-t-xl">
            <div className="flex items-center gap-3 mb-1">
              <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-brand-green">
                <HiOutlineUserAdd size={20} />
              </div>
              <h2 className="text-lg font-bold text-brand-green-900">New Sub-Account</h2>
            </div>
            <p className="text-xs text-brand-green-500 ml-11">Assign specific administrative powers.</p>
          </div>
          ...
          <form onSubmit={handleCreateAccount} className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-brand-green-200 bg-brand-green-50 text-sm focus:ring-2 focus:ring-emerald-500/20 outline-none"
            ...
            <div className="pt-2">
              <label className="block text-[10px] font-black text-brand-green-500 uppercase tracking-widest mb-3">Assign Access</label>
              <div className="space-y-2">
                {AVAILABLE_ROLES.map(role => (
                  <label key={role.id} className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-all group ${newUser.roles.includes(role.name) ? 'border-brand-green bg-emerald-50/50' : 'border-brand-green-100 hover:bg-brand-green-50'
                    }`}>
                    <input
                      type="checkbox"
                      className="mt-1 rounded border-brand-green-300 text-brand-green focus:ring-brand-green"
                      checked={newUser.roles.includes(role.name)}
                      onChange={() => toggleRoleSelection(role.name)}
                    />
                    <div className="flex-1">
                      <span className="block text-sm font-bold text-brand-green-900">{role.name}</span>
                      <span className="block text-[11px] text-brand-green-500 leading-tight">{role.description}</span>
                    </div>
                  </label>
                ))}
              </div>
            ...
            <div className="pt-4 border-t border-brand-green-100 flex gap-3">
              <button
                type="button"
                onClick={() => setNewUser({ name: '', email: '', roles: [] })}
                className="flex-1 px-4 py-2.5 border border-brand-green-200 rounded-lg text-sm font-bold text-brand-green-600 hover:bg-brand-green-50 transition-colors"
              >
                Reset
              </button>
              <button
