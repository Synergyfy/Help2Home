...
interface EducationPageContentProps {
    role: 'landlord' | 'agent' | 'developer' | 'caretaker' | 'investor' | 'admin' | 'superAdmin';
}

export default function EducationPageContent({ role }: EducationPageContentProps) {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                // Filter by role
                const filteredByRole = (data: EducationContent[]) => {
                    if (role === 'admin' || role === 'superAdmin') return data;
                    return data.filter(c => c.targetAudience.includes('all') || c.targetAudience.includes(role as any));
                };

                setContents(filteredByRole(contentData));
                setFeatured(filteredByRole(featuredData));
            } catch (error) {
    ...
    let pageDescription = "Explore resources and guides to enhance your knowledge.";

    switch (role) {
        case 'landlord':
            pageDescription = "Resources and guides to help you manage your properties and tenants effectively.";
            break;
        case 'agent':
            pageDescription = "Access valuable insights and training to boost your real estate career.";
            break;
        case 'developer':
            pageDescription = "Stay informed with best practices and market trends for property development.";
            break;
        case 'caretaker':
            pageDescription = "Learn essential skills for property maintenance and tenant communication.";
            break;
        case 'investor':
            pageDescription = "Deepen your understanding of real estate investments and market strategies.";
            break;
        case 'admin':
            pageDescription = "Find guides and policies for effective platform administration.";
            break;
        case 'superAdmin':
            pageDescription = "Comprehensive documentation for system management and strategic oversight.";
            break;
    }
